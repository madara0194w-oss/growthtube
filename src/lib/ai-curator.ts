 ...
 */
export async function evaluateVideo(video: VideoMetadata): Promise<VideoEvaluation> {
  try {
...
export function shouldAutoApprove(evaluation: VideoEvaluation): boolean {
  // Approve everything - AI will always approve now
  return true
}
...
export function shouldReview(evaluation: VideoEvaluation): boolean {
  // No review needed - everything auto-approves
  return false
}
